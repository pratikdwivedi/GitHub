	...
	public void userPrivilegesTest() {
		...
		try {
			...
			for (int userCount = 1; userCount <= lib.rowCount; userCount++) {
				...
				if(user!=null)
				{
				String pwd = lib.getExcelData("users", userCount, 1, filePath);
				System.out.println("S.No.->" + lib.rowCount);
				String browser=lib.getExcelData("browsers",1,0,"SapphireFiles\\AutomationTestData.xls");
				String url=lib.getExcelData("browsers",1,1,"SapphireFiles\\AutomationTestData.xls");
				...
				FileUtils.copyFile(srcfile,new File("D:\\ScreenShots\\Admin\\"+user+".png"));
				act = new Actions(driver);
				driver.findElement(By.linkText("WORKSPACE")).click();
				List<WebElement> menu=driver.findElements(By.xpath("//div[contains(@id,'headerMenu')]//ul/li/a/span[1]"));
				...
				{
				String userPrivilegesAdmin = lib.getExcelData("userPrivileges", i, userCount, filePath);
				listUserPri.add(userPrivilegesAdmin);
				}
				System.out.println(listUserPri);
				System.out.println(listHeaderItems.containsAll(listUserPri));
				Thread.sleep(2000);
				...
				System.out.println("Login Logout successfully with user ............"+ user);
				Reporter.log("Login Logout successfully with user............."+ user);
				driver.close();
				driver.quit();
				}
			}
