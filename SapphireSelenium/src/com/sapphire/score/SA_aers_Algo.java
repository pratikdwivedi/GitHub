		...
		try {
			...
			for (int i = 1; i <= lib1.rowCount; i++) {
				...
				if (analysisType != null) {
					String drugName = lib1.getExcelData("DECCaseCount", i, 3,
							filePath);
					String event = lib1.getExcelData("DECCaseCount", i, 4,
							filePath);
					String DBTableName = lib1.getExcelData("DECCaseCount", i,
							8, filePath);
					if (drugName != null && event != null
							&& DBTableName != null) {
						String query = "SELECT exp_A,rr,prr_lb,prr_ub,ebgm_lb,ebgm_ub,ror_lb,ror_ub,bcpnn_lb,bcpnn_ub,chi_sqr_ABCD,chi_sqr_yates_ABCD,yulesq_lb,yulesq_ub FROM "
								+ DBTableName
								+ " where drug_name='"
								+ drugName
								+ "' and event_name ='"
						...
						if (analysisType != null) {
							...
							Thread.sleep(10000);
							List<WebElement> hierarchy = driver
									.findElements(By
						...
						System.out
								.println("Some important data in test data file is missing....");
