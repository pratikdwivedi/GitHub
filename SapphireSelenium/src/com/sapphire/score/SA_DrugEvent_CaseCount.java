...
public class SA_DrugEvent_CaseCount extends Config{
	...
	public void SADrugEvent_CaseCount() throws Exception{
		try{
			...
			{
				String drugName=lib1.getExcelData("SADECCaseCount", i, 3, filePath);
				String event=lib1.getExcelData("SADECCaseCount", i, 4, filePath);
				String DBColumnName=lib1.getExcelData("SADECCaseCount", i,8, filePath);
				 Class.forName(dbc.dbClass);
				 Connection con = DriverManager.getConnection (dbc.dbUrl,dbc.username,dbc.password);
				 Statement stmt = con.createStatement();
				 String query = "select count(distinct isr_id) from aers.fact_demo_drugs_reacs where "+DBColumnName+"='"+drugName+"' and pt_name='"+event+"';";
			...
			String analysisType=lib1.getExcelData("SADECCaseCount", i, 0, filePath);
			...
			act.perform();
			Thread.sleep(5000);
			String drugLevel=lib1.getExcelData("SADECCaseCount", i, 1, filePath);
			...
			Thread.sleep(3000);
				...
				String dictLevel=lib1.getExcelData("SADECCaseCount", i, 2, filePath);
				...
				String filter=lib1.getExcelData("SADECCaseCount", i, 5, filePath);
