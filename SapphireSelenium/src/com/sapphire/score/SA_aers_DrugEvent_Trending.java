	...
	public void SADrugEvent_Algo() throws Exception {
		try {
			...
			for (int i = 1; i <= lib1.rowCount; i++) {
				...
				String DBTableName = lib1.getExcelData("DECCaseCount", i, 8,
						filePath);
				String query = "SELECT prr_lb FROM "
						+ DBTableName
						+ " where drug_name='"
						+ drugName
						+ "' and event_name ='"
						+ event
						+ "' and fda_period<=(SELECT * FROM aers.stat_latest_quarter) and fda_period>=(SELECT quarter as Q FROM aers.stat_latest_quarter) - 3 order by fda_period;";
