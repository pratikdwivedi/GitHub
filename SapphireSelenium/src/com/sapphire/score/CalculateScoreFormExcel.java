	...
	public void scoreTestExcel() {
		...
		try {
			...
			for (int i = 1; i <= lib1.rowCount; i++) {
				 ...
				 */
				double ExpA = wd.readExcelEvaluation("Sheet1", "E27", filePath);
				double PRR_LB = wd.readExcelEvaluation("Sheet1", "E35",
						filePath);
				double PRR = wd.readExcelEvaluation("Sheet1", "E33", filePath);
				double PRR_UB = wd.readExcelEvaluation("Sheet1", "E36",
						filePath);
				System.out.println("Exp A :" + ExpA);
				System.out.println("PRR (-) : " + PRR_LB);
				System.out.println("PRR : " + PRR);
				System.out.println("PRR (+) : " + PRR_UB);
				...
				Assert.assertEquals(ExpA, dbStatvalueExpA);
				Assert.assertEquals(PRR_UB, dbStatvaluePRRLB);
				Assert.assertEquals(PRR, dbStatvaluePRR);
				Assert.assertEquals(PRR_UB, dbStatvaluePRRUB);
