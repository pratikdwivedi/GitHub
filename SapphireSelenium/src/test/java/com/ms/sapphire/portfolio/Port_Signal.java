...
public class Port_Signal extends Config {
	@Test
	public void master_siganls() throws Exception {
		...
		try {
			...
			for (int i = 1; i <= lib1.rowCount; i++) {
				...
				String masterName = lib1
						.getExcelData("Signals", i, 0, filePath);
				...
				Thread.sleep(3000);
				driver.findElement(
						By.xpath("//form[contains(@id,'portLeftPane')]//div[1]/ul/li/a/span[contains(text(),'Signals')]"))
						.click();
				String annotation = lib1
						.getExcelData("Signals", i, 1, filePath);
				driver.findElement(
						By.id("masterSignalForm:signalGrid:annotationTypeId_label"))
						.click();
				driver.findElement(
						By.xpath("//div[contains(@id,'masterSignalForm:signalGrid:annotationTypeId_panel')]//ul/li[contains(text(),'"
								+ annotation + "')]")).click();
				Thread.sleep(3000);
				String event = lib1.getExcelData("Signals", i, 2, filePath);
				if (event != null) {
					driver.findElement(
							By.xpath("//div[contains(@id,'masterSignalForm:signalGrid')]//div/span[2]/input"))
							.sendKeys(event);
				}
				...
				if (fromMonth != null && fromYear != null && fromDate != null) {
					driver.findElement(
							By.id("masterSignalForm:signalGrid:startDateId"))
							.click();
					Select monthList = new Select(
							driver.findElement(By
									.xpath("//div[contains(@id,'ui-datepicker-div')]//div/div/select[1]")));
					monthList.selectByVisibleText(fromMonth);
					int fromYearInt = Double.valueOf(fromYear).intValue();
					Select yearList = new Select(
							driver.findElement(By
									.xpath("//div[contains(@id,'ui-datepicker-div')]//div/div/select[2]")));
					yearList.selectByVisibleText("" + fromYearInt);
					int fromDateInt = Double.valueOf(fromDate).intValue();
					driver.findElement(
							By.xpath("//div[contains(@id,'ui-datepicker-div')]//td/a[contains(text(),'"
									+ fromDateInt + "')]")).click();
				}
				...
				if (toMonth != null && toYear != null && toDate != null) {
					driver.findElement(
							By.id("masterSignalForm:signalGrid:endDateId"))
							.click();
					Select toMonthList = new Select(
							driver.findElement(By
									.xpath("//div[contains(@id,'ui-datepicker-div')]//div/div/select[1]")));
					toMonthList.selectByVisibleText(toMonth);
					int toYearInt = Double.valueOf(toYear).intValue();
					Select toYearList = new Select(
							driver.findElement(By
									.xpath("//div[contains(@id,'ui-datepicker-div')]//div/div/select[2]")));
				...
				driver.findElement(
						By.xpath("//div[contains(@id,'masterSignalForm:signalGrid')]//div/span[2]/button[1]"))
						.click();
				List<WebElement> annotationList = driver
						.findElements(By
								.xpath("//tbody[contains(@id,'masterSignalForm:signalGrid_data')]//div"));
