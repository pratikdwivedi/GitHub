...
public class Port_Create_SA_cs extends Config {
	@Test
	public void create_sa_cs() throws Exception {
		...
		try {
			...
			for (int i = 1; i <= lib1.rowCount; i++) {
				...
				String alertTypeSAcs = lib1.getExcelData("sa-cs", i, 2,
						filePath);
				WebElement weAlert = driver
						.findElement(By
								.xpath("//div[contains(@id,'tabView:topicDetailForm:tabView:analysisGrid:startAnalysisPanel')]//td[2]//label[contains(text(),'"
										+ alertTypeSAcs + "')]"));
				act.moveToElement(weAlert).click();// FDA AERS clicked
				act.perform();
				Thread.sleep(3000);
				driver.findElement(By.id("analysisCriteriaForm:lookupUdr"))
						.click();
				driver.switchTo().activeElement();
				String udrSAcs = lib1.getExcelData("sa-cs", i, 3, filePath);
				if (udrSAcs != null) {
					driver.findElement(
							By.xpath("//div[contains(@class,'ui-toolbar-group-left mergeInHeader')]//span/input"))
							.sendKeys(udrSAcs);
					driver.findElement(
							By.xpath("//div[contains(@class,'ui-toolbar-group-left mergeInHeader')]//span/button[1]/span"))
							.click();
					Thread.sleep(2000);
					driver.findElement(
							By.xpath("//tr[td[2][div[contains(text(),'"
									+ udrSAcs + "')]]]//td[1]/div/div/div[2]"))
							.click();
					driver.findElement(By.id("udrLookupForm:selectUdr"))
							.click();
				}
