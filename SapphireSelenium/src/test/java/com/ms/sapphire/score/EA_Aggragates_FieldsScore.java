	...
	public void EA_Aggregates_FieldsScore() throws Exception {
		try {
			...
			for (int i = 1; i <= lib1.rowCount; i++) {
				...
				drugName = lib1.getExcelData(
						"EA_Aggreagtes_FieldsScore", i, 3, filePath);
				analysisType = lib1.getExcelData(
						"EA_Aggreagtes_FieldsScore", i, 0, filePath);
				event = lib1.getExcelData("EA_Aggreagtes_FieldsScore",
						i, 5, filePath);
				dbc.dbConnect();
				query = "select * from (select 'Death' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_death = 1 group by fddr.outc_death union all select 'Life Threatening' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_life_threat = 1 group by fddr.outc_life_threat union all select 'Hospitalization' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers.aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_hosp = 1 group by fddr.outc_hosp union all select 'Disability' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_disability = 1 group by fddr.outc_disability union all select 'Congenital Anomaly' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_cong_anomaly = 1 group by fddr.outc_cong_anomaly union all select 'Required Intervention' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_reqd_interv = 1 group by fddr.outc_reqd_interv union all select 'Other' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_other = 1 group by fddr.outc_other union all select 'None' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_none = 1 group by fddr.outc_none union all select 'Recovered' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_rec = 1 group by fddr.outc_rec union all select 'Treated with drug' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
						+ "' and fddr.outc_rx = 1 group by fddr.outc_rec union all select 'Not available' outcome, count(distinct fddr.isr_id) cnt FROM fact_demo_drugs_reacs fddr,aers_drug_ings adi where fddr.isr_id = adi.isr_id and fddr.trade_name='"
						+ drugName
		...
		/*		Assert.assertEquals(actual, expected); */
			...
			Assert.fail("EA fields values is incorrect for "+drugName);
		} finally {
			Reporter.log("This test case is to verify EA field values in application and in database fact_demo_drugs_reacs table");
			Reporter.log("Expected : "+dbListvalue);
			Reporter.log("Actual : "+GroupList);
			Reporter.log(query);
			dbc.stmt.close();
			dbc.con.close();
		}
